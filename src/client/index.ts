...
window.addEventListener("message", (event) => {
  if (event.data.type === "vegas:gasinit") {
    vegasWindow.vegas.host = event.data.payload.host;
                  ...
                  get: (_target, property, receiver) => {
                    if (property === "withSuccessHandler") {
                      ...
                      return (...args: any[]) => {
                        ...
                        } while (vegasWindow.vegas.requestMap.has(requestId));
                        vegasWindow.vegas.requestMap.set(requestId, receiver);
                        window.parent.postMessage(
                          {
                            type: "vegas:gascall",
                            payload: { id: requestId, func: property, args: JSON.stringify(args) },
                          },
                          vegasWindow.vegas.host,
                        );
                      };
                    }
                  },
    ...
    const gasRun = vegasWindow.vegas.requestMap.get(event.data.payload.id);
    ...
    vegasWindow.vegas.requestMap.delete(event.data.payload.id);
  }
});

Object.assign(window, vegasWindow);
