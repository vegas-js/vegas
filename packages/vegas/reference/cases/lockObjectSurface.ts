...
function describeLock(value: object) {
  return {
    type: typeof value,
    ...
    methodOwnership: {
      hasLock: describePropertyOwnership(value, "hasLock"),
      releaseLock: describePropertyOwnership(value, "releaseLock"),
      tryLock: describePropertyOwnership(value, "tryLock"),
      waitLock: describePropertyOwnership(value, "waitLock"),
...
export function captureReferenceLockObjectSurface() {
  const globals = globalThis as unknown as Record<string, any>;
  const lockService = globals.LockService;

  return {
    scriptLock: describeLock(lockService.getScriptLock()),
    userLock: describeLock(lockService.getUserLock()),
    privateLock: describeLock(lockService.getPrivateLock()),
    publicLock: describeLock(lockService.getPublicLock()),
  };
}
