...
function describeCache(value: object) {
  return {
    type: typeof value,
    ...
    methodOwnership: {
      get: describePropertyOwnership(value, "get"),
      getAll: describePropertyOwnership(value, "getAll"),
      put: describePropertyOwnership(value, "put"),
      putAll: describePropertyOwnership(value, "putAll"),
      remove: describePropertyOwnership(value, "remove"),
      removeAll: describePropertyOwnership(value, "removeAll"),
...
export function captureReferenceCacheObjectSurface() {
  const globals = globalThis as unknown as Record<string, any>;
  const cacheService = globals.CacheService;

  return {
    scriptCache: describeCache(cacheService.getScriptCache()),
    userCache: describeCache(cacheService.getUserCache()),
    privateCache: describeCache(cacheService.getPrivateCache()),
    publicCache: describeCache(cacheService.getPublicCache()),
  };
}
