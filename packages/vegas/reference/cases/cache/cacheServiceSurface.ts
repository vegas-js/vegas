...
export function captureReferenceCacheServiceSurface() {
  const globals = globalThis as unknown as Record<string, unknown>;
  const cacheService = globals.CacheService;

  if (
    cacheService === null ||
    (typeof cacheService !== "object" && typeof cacheService !== "function")
  ) {
    return {
      properties: null,
  ...
  return {
    properties: describeProperties(cacheService),
    prototypeIsObjectPrototype: Object.getPrototypeOf(cacheService) === Object.prototype,
    stringify: String(cacheService as any),
  };
}
