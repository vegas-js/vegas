...
export function captureReferenceUtilitiesSurface() {
  ...
  const enumLikePropertyNames = findObjectPropertyNames(utilities);

  return {
    type: typeof utilities,
    properties: describeProperties(utilities),
    prototypeIsObjectPrototype: Object.getPrototypeOf(utilities) === Object.prototype,
    stringify: String(utilities as any),
    enumLikeProperties: enumLikePropertyNames.map((name) => {
      const value = (utilities as Record<string, unknown>)[name];

      if (value === null || typeof value !== "object") {
        throw new Error(`Invalid enum-like property: Utilities.${name}`);
      }

      return describeEnumLikeProperty(`Utilities.${name}`, value);
    }),
  };
}
