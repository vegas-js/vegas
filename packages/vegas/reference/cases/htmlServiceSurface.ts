...
function describeEnumLikeProperty(name: string, container: object) {
  ...
  const compareTo = Object.fromEntries(
    memberNames.map((sourceName) => {
      const source = containerRecord[sourceName];

      if (source === null || typeof source !== "object") {
        throw new Error(`Invalid enum member: ${name}.${sourceName}`);
      }
      ...
      return [
        ...
        Object.fromEntries(
          memberNames.map((targetName) => {
            const target = containerRecord[targetName];

            return [targetName, callMethod(source, "compareTo", [target])];
          }),
        ),
      ];
    }),
...
export function captureReferenceHtmlServiceSurface() {
  ...
  const enumLikePropertyNames = findObjectPropertyNames(htmlService);

  return {
    type: typeof htmlService,
    properties: describeProperties(htmlService),
    prototypeIsObjectPrototype: Object.getPrototypeOf(htmlService) === Object.prototype,
    stringify: String(htmlService as any),
    enumLikeProperties: enumLikePropertyNames.map((name) => {
      const value = (htmlService as Record<string, unknown>)[name];

      if (value === null || typeof value !== "object") {
        throw new Error(`Invalid enum-like property: HtmlService.${name}`);
      }

      return describeEnumLikeProperty(`HtmlService.${name}`, value);
    }),
  };
}
