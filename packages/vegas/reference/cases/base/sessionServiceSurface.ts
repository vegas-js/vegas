...
export function captureReferenceSessionServiceSurface() {
  const globals = globalThis as unknown as Record<string, unknown>;
  const sessionService = globals.Session;

  if (
    sessionService === null ||
    (typeof sessionService !== "object" && typeof sessionService !== "function")
  ) {
    return {
      properties: null,
  ...
  return {
    properties: describeProperties(sessionService),
    prototypeIsObjectPrototype: Object.getPrototypeOf(sessionService) === Object.prototype,
    stringify: String(sessionService as any),
  };
}
