...
function describeUser(value: object) {
  const email = (value as any).getEmail();

  return {
    type: typeof value,
    stringifyEqualsEmail: String(value as any) === email,
    ...
    methodOwnership: {
      getEmail: describePropertyOwnership(value, "getEmail"),
      getUserLoginId: describePropertyOwnership(value, "getUserLoginId"),
      getUsername: describePropertyOwnership(value, "getUsername"),
...
export function captureReferenceUserObjectSurface() {
  const globals = globalThis as unknown as Record<string, any>;
  const user = globals.Session.getActiveUser();
