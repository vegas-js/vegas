...
const PI_SUBSTITUTION = Uint8Array.from([
...
function appendPadding(data: Uint8Array): Uint8Array {
...
function computeChecksum(data: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < data.length; offset += 16) {
    for (let index = 0; index < 16; index++) {
      ...
      checksum[index] ^= PI_SUBSTITUTION[value ^ previous];
...
export function computeMd2(data: Uint8Array): Uint8Array {
  const padded = appendPadding(data);
  const message = new Uint8Array(padded.length + 16);

  message.set(padded);
  message.set(computeChecksum(padded), padded.length);

  const state = new Uint8Array(48);

  for (let offset = 0; offset < message.length; offset += 16) {
    for (let index = 0; index < 16; index++) {
      state[index + 16] = message[offset + index];
      state[index + 32] = state[index] ^ state[index + 16];
    }

    let temporary = 0;
    for (let round = 0; round < 18; round++) {
      for (let index = 0; index < 48; index++) {
        state[index] ^= PI_SUBSTITUTION[temporary];
        temporary = state[index];
      }
      temporary = (temporary + round) & 0xff;
    }
  }
