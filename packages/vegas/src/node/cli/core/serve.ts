...
import { launchGAS, launchWebApp } from "./launch";
    ...
    if (request.url) {
      ...
      if (url.pathname === "/") {
        ...
        const handled = await handleWebAppHttpRequest({
          ...
          execute: (webAppRequest) => launchWebApp(ctx, webAppRequest),

          renderHtml: async (renderUrl, result) => {
            let uuid = "";
            ...
            const html = createHostHtml(renderUrl, result);

            return hostServer.transformIndexHtml(renderUrl.href, html);
          },
        });
