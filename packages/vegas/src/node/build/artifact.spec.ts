...
describe("replaceOutputArtifacts", () => {
  test("replace output without previous output", async () => {
    const tempDirPath = fs.mkdtempSync(path.join(os.tmpdir(), "vegas-"));
    const outputDir = path.join(tempDirPath, "dist");
    ...
    try {
      ...
      expect(fs.readFileSync(path.join(outputDir, "Code.js"), "utf8")).toBe("next");
      expect(
        fs
  ...
  test("propagate unexpected error while checking previous output", async () => {
    const tempDirPath = fs.mkdtempSync(path.join(os.tmpdir(), "vegas-"));
    const outputDir = path.join(tempDirPath, "dist");
    const error = Object.assign(new Error("rename failed"), {
      code: "EACCES",
    });
    ...
    try {
      ...
      using renameMock = vi.spyOn(fs.promises, "rename").mockRejectedValue(error);
